    ...
    (function (blocks) {
        ...
        function setOutputCheck(block, retType, info) {
            switch (retType) {
                ...
                default:
                    if (retType !== "T") {
                        var opt_check = isArrayType(retType) ? ["Array"] : [];
                        var si_r = info.apis.byQName[retType];
        ...
        function initFunctions() {
            ...
            if (functionOptions) {
                if (functionOptions.useNewFunctions) {
                    ...
                    var oldFlyout_1 = Blockly.Functions.flyoutCategory;
                    Blockly.Functions.flyoutCategory = function (workspace) {
                        var elems = oldFlyout_1(workspace);
                        var headingLabel = createFlyoutHeadingLabel(lf("Functions"), pxt.toolbox.getNamespaceColor('functions'), pxt.toolbox.getNamespaceIcon('functions'), 'blocklyFlyoutIconfunctions');
                ...
                if (functionOptions.extraFunctionEditorTypes) {
                    functionOptions.extraFunctionEditorTypes.forEach(function (t) {
                        iconsMap_1[t.typeName] = t.icon || pxt.blocks.defaultIconForArgType();
                        if (t.defaultName) {
                            customNames_1[t.typeName] = t.defaultName;
                        }
                    });
                }
                Blockly.PXTBlockly.FunctionUtils.argumentIcons = iconsMap_1;
                Blockly.PXTBlockly.FunctionUtils.argumentDefaultNames = customNames_1;
            }
